...
interface StreamWriter {
    ...
    suspend fun write(filteredMessage: PipelineFilteredMessage, counter: AtomicLong)
...
//        )
    ...
    override suspend fun write(filteredMessage: PipelineFilteredMessage, counter: AtomicLong) {
        val result = CompletableDeferred<List<StreamResponse>>()
        responses.send(result)
        ...
        writerPool.launch {
            result.complete(
                let {
                    val convertedMessage = measureTimedValue {
                        StreamResponse.newBuilder()
                            .setMessage(MessageMapper.convertToGrpcMessageData(filteredMessage.payload))
                            .build()
                    }
                    filteredMessage.info.serializingTime = convertedMessage.duration.toLongMilliseconds()
                    StreamWriter.setSerializing(filteredMessage.info)

                    listOf(convertedMessage.value)
                }
            )
            counter.incrementAndGet()
        }
