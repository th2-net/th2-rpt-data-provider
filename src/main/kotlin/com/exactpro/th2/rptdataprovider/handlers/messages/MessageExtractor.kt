            ...
            if (resumeFromId == null || resumeFromId.hasStarted) {
                val cradleMessageIterable = context.cradleService.getMessagesBatchesSuspend(
                    StoredMessageFilterBuilder()
                        ...
                        .also { builder ->
                            if (resumeFromId?.lastId != null) {
                                builder.index().let {
                                    val index = resumeFromId.lastId.index
                ...
                for (batch in cradleMessageIterable) {
                    if (externalScope.isActive) {
                        ...
                        logger.trace {
                            "batch ${batch.id.index} of stream $streamName has been trimmed (targetStartTimestamp=${request.startTimestamp} targetEndTimestamp=${request.endTimestamp} targetId=${resumeFromId?.lastId?.index}) - ${trimmedMessages.size} of ${batch.messages.size} messages left (firstId=${firstMessage.id.index} firstTimestamp=${firstMessage.timestamp} lastId=${lastMessage.id.index} lastTimestamp=${lastMessage.timestamp})"
                        }
